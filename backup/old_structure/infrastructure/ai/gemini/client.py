...
class GeminiModel(AIModelPort):
    """
    Modelo Gemini:
    - NO transcribe audio (raises NotImplementedError)
    ...
    def transcribe(self, audio_path: str) -> str:
        """Gemini does not support audio transcription."""
        raise NotImplementedError("GeminiModel does not support audio transcription")

    def run_agent(self, mode: str, text: str) -> str:
        agent = self.agents.get(mode, default_agent)

        result = agent.run(text, stream=False)
